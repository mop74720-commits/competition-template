...
errors=[]
if (ROOT/'VERSION').read_text(encoding='utf-8').strip()!='0.1.2': errors.append('VERSION != 0.1.2')
if '# Competition Repository Template v0.1.2' not in (ROOT/'README.md').read_text(encoding='utf-8'): errors.append('README version mismatch')
for rel in ['DESIGN_GUARDRAILS.md','scripts/contest_import.py','problem/SOURCE_MANIFEST.csv','problem/PREFLIGHT_REPORT.md','scripts/add_question.py','scripts/new_run.py','scripts/validate_repo.py','rules/OFFICIAL_RULES.md','rules/RULE_PROFILE.json','scripts/validate_submission.py','scripts/build_support.py','audit/SUBMISSION_MANIFEST.csv','audit/SUPPORT_MANIFEST.csv','ai/AI_USAGE_DETAILS_TEMPLATE.md']:
...
print('TEMPLATE_VALIDATION_PASS: dynamic questions + preflight + official-rules/submission closure + state-first guardrails')
